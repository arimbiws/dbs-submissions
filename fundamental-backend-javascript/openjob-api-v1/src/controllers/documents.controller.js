...
const uploadDocument = async (req, res, next) => {
  try {
    if (!req.file) throw new ClientError("File document tidak ditemukan atau salah format");
    ...
    const result = await pool.query(query, [id, userId, filename, path]);
...
const getDocuments = async (req, res, next) => {
  try {
    const result = await pool.query("SELECT id, user_id, filename, path FROM documents");
    res.status(200).json({
      status: "success",
...
const getDocumentById = async (req, res, next) => {
  try {
    const result = await pool.query("SELECT id, user_id, filename, path FROM documents WHERE id = $1", [req.params.id]);
